...
public class ListController {
    ...
    @GetMapping
    public CommonResponse<PageDTO<ListSummaryDTO>> getLists(
            @PageableDefault(page = 0, size = 20) Pageable pageable
    ) {
        PageDTO<ListSummaryDTO> data = listQueryService.getLists(pageable);
        return CommonResponse.ok(data, "리스트 조회 성공");
    }

    @PostMapping
    public CommonResponse<ListSimpleDTO> create(
            @Valid @RequestBody CreateListRequest req
    ) {
        ListSimpleDTO data = listCommandService.create(req.getName(), req.getEmoji());
        return CommonResponse.created(data, "리스트가 생성되었습니다.");
    }

    @PatchMapping("/{listId}")
    public CommonResponse<ListSimpleDTO> update(
            @PathVariable Long listId,
            @Valid @RequestBody UpdateListRequest req
    ) {
        ListSimpleDTO data = listCommandService.update(listId, req.getName(), req.getEmoji());
        return CommonResponse.ok(data, "리스트가 수정되었습니다.");
    }

    @DeleteMapping("/{listId}")
    public CommonResponse<Map<String, Object>> delete(
            @PathVariable Long listId
    ) {
        listCommandService.delete(listId);
        return CommonResponse.ok(Map.of("id", listId), "리스트가 삭제되었습니다.");
    }

    @PostMapping("/{listId}/places")
    public CommonResponse<Map<String, Object>> addPlace(
            @PathVariable Long listId,
    ...
    public CommonResponse<Map<String, Object>> removePlace(
            @PathVariable Long listId,
